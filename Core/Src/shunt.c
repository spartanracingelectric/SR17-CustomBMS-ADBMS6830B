...
static int32_t lastShuntMessageTime = 0;
...
void Shunt_handleCANMessage_Current(uint8_t rxData[8])
{
	...
	lastShuntMessageTime = HAL_GetTick();
}
...
{
	int32_t currentTime_ms = HAL_GetTick();

	if (currentTime_ms - lastShuntMessageTime > 3000)
	{
		shunt.current_mA = 0;
		shunt.coulombCount = 0;
	}
...
static int32_t parseInt32LE(uint8_t *data)
{
	return (int32_t)(((int32_t)data[0] << 0) |
					 ((int32_t)data[1] << 8) |
					 ((int32_t)data[2] << 16) |
					 ((int32_t)data[3] << 24));
}
...
static int64_t parseInt64LE(uint8_t *data)
{
	return (int64_t)(((int64_t)data[0] << 0) |
					 ((int64_t)data[1] << 8) |
					 ((int64_t)data[2] << 16) |
					 ((int64_t)data[3] << 24) |
					 ((int64_t)data[4] << 32) |
