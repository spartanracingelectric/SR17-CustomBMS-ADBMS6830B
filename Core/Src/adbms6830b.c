...
{
	// TODO: Error check
	HAL_SPI_Receive(&hspi1, (uint8_t *)&rxBuffer[0][0], NUM_MODULES_TOTAL * (DATA_LENGTH_BYTES + PEC_LENGTH_BYTES), 100);
}

void ADBMS_sendCommand(uint16_t command)
{
	uint8_t txBuffer[COMMAND_LENGTH_BYTES + PEC_LENGTH_BYTES];
	txBuffer[0] = (uint8_t)(command >> 8);
	txBuffer[1] = (uint8_t)(command);

	uint16_t pec = ADBMS_calculateCommandPec(txBuffer, COMMAND_LENGTH_BYTES);
	txBuffer[2] = (uint8_t)(pec >> 8);
	txBuffer[3] = (uint8_t)(pec);

	HAL_SPI_Transmit(&hspi1, txBuffer, COMMAND_LENGTH_BYTES + PEC_LENGTH_BYTES, 100);
}
...
void ADBMS_getCellVoltages(ModuleData module[NUM_MODULES_TOTAL])
{
	...
	ADBMS_snap();
	int numberOfRegisters = (NUM_CELLS_PER_MODULE + (CELLS_PER_ADC_REGISTER - 1)) / CELLS_PER_ADC_REGISTER;
...
void ADBMS_parseCellVoltages(uint8_t rxBuffer[NUM_MODULES_TOTAL][REGISTER_LENGTH_BYTES], uint8_t registerIndex, ModuleData module[NUM_MODULES_TOTAL])
{
	...
	{
		bool isDataValid = ADBMS_checkDataPec(&rxBuffer[moduleIndex][0], DATA_LENGTH_BYTES, &rxBuffer[moduleIndex][DATA_LENGTH_BYTES]);
		...
		for (uint8_t cellOffset = 0; cellOffset < CELLS_PER_ADC_REGISTER; cellOffset++)
		{
			...
			if (cellIndex > NUM_CELLS_PER_MODULE - 1)
				break;
...
{
	uint8_t txBuffer[NUM_MODULES_TOTAL][DATA_LENGTH_BYTES + PEC_LENGTH_BYTES];

	for (int moduleIndex = 0; moduleIndex < NUM_MODULES_TOTAL; moduleIndex++)
	{
		memcpy(txBuffer[moduleIndex], data[moduleIndex], DATA_LENGTH_BYTES);

		uint16_t dataPec = ADBMS_calculateDataPec(data[moduleIndex], DATA_LENGTH_BYTES, 0);
		txBuffer[moduleIndex][DATA_LENGTH_BYTES + 0] = (uint8_t)(dataPec >> 8);
		txBuffer[moduleIndex][DATA_LENGTH_BYTES + 1] = (uint8_t)(dataPec);
	}
...
void ADBMS_getGpioVoltages(ModuleData module[NUM_MODULES_TOTAL])
{
	...
	int numberOfRegisters = NUM_AUX_REGISTERS;
	for (uint8_t registerIndex = 0; registerIndex < numberOfRegisters; registerIndex++)
	{
		isoSPI_Idle_to_Ready();
		ADBMS_csLow();
